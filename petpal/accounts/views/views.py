    ...
    def get_object(self):
        ...
        try:
            if self.request.user.shelter:
                check_app = Application.objects.filter(
                    applicant=seeker, pet_listing__shelter=self.request.user
                ).exists()
                ...
                if check_app:
                    ...
                    raise ValidationError(
                        {
                            "detail": "You cannot view this seeker since it does not have any active applications with you."
                        }
                    )
        except Shelter.DoesNotExist:
            try:
                if self.request.user.seeker:
                    if self.request.user.seeker.id == self.kwargs["pk"]:
                        return seeker
                    else:
                        raise ValidationError(
                            {"detail": "You cannot view this seeker."}
                        )
